/**
 * @file CosmicQuestLogic.test.ts
 * @purpose Unit tests for Cosmic Quest slot game logic.
 * @author C-3PO
 * @date 2026-04-14
 * @license Proprietary – available for licensing
 */

import { createCosmicQuestState, spinCosmicQuest, simulateCosmicQuestRTP, CosmicQuestSymbol } from '../games/CosmicQuestLogic';

describe('CosmicQuestLogic', () => {
  ...
  it('should create an initial game state correctly', () => {
    const state = createCosmicQuestState(BET_PER_LINE, LINES_BET);
  ...
  it('should execute a spin and update reel stops', () => {
    ...
    spinCosmicQuest(state, { rng: mockRNG });
  ...
  it('should award free spins when 3 or more scatters appear', () => {
    const state = createCosmicQuestState(BET_PER_LINE, LINES_BET);
    // Use overrideReelStops to guarantee the scatter positions for testing
    const mockReelStops: CosmicQuestSymbol[][] = [
      ['SCATTER', '10', 'J'],
      ['10', 'SCATTER', 'Q'],
      ['A', 'K', 'SCATTER'],
      ['10', 'J', 'Q'],
    ...
    state.freeSpinsRemaining = 0; // Ensure fresh start

    spinCosmicQuest(state, { rng: mockRNG, houseEdge: HOUSE_EDGE, overrideReelStops: mockReelStops });

    // Cosmic Quest awards 12 free spins, but 1 is used immediately.
    expect(state.freeSpinsRemaining).toBe(11);
  ...
  it('should decrement free spins and set isComplete when no free spins remain', () => {
    ...
    spinCosmicQuest(state, { rng: mockRNG, houseEdge: HOUSE_EDGE });
  ...
  it.skip('should estimate RTP around 96% with many rounds', () => {
    ...
    const estimatedRTP = simulateCosmicQuestRTP(rounds, BET_PER_LINE, LINES_BET, { houseEdge: HOUSE_EDGE });
